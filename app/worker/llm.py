...
def classify_categories(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if unc.empty:
        return df
    rows_text = "\n".join(f'{i+1}. txn_id={r["txn_id"]}, merchant="{r["merchant"]}", amount={r["amount"]}, currency={r["currency"]}, notes="{r["notes"]}"' for i, (_, r) in enumerate(unc.iterrows()))
